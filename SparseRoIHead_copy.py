...
from HEAD import MaskRCNNHeads
...
def roi_align(cut_masks, output_size):
    """
    ...
    """
    # N, H, W = cut_masks.size()
    output_height, output_width = output_size

    # 使用双线性插值调整掩码大小
    resized_masks = F.interpolate(
        cut_masks.unsqueeze(1).float(),  # 增加一个维度以符合F.interpolate的输入要求
        size=(output_height, output_width),
        mode="bilinear",  # 使用双线性插值
        align_corners=False,
    )  # 对齐角落的设置

    return resized_masks.squeeze(1)  # 移除增加的维度，返回调整大小后的掩码
...
class SparseRoIHeads(nn.Module):
    __annotations__ = {
    ...
    def __init__(
        ...
        #     mask_dilation = 1
        #     mask_head = MaskRCNNHeads(2048, mask_layers, mask_dilation)
